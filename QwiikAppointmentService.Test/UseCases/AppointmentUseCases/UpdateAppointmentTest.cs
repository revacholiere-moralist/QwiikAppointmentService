...
namespace QwiikAppointmentService.Test.UseCases.AppointmentUseCases
{
    ...
    {
        [Fact]
        public async Task UpdateAppointment_ShouldPass()
        {
            // arrange
            ...
            var mockedAppointmentResponse = new Appointment()
            {
                ...
                CreatedById = 2,
                LastUpdatedById = 2,
                CreatedDate = utcNow.AddHours(-1),
                LastUpdatedDate = utcNow.AddHours(-1),
                IsActive = true
            };

            var mockedAppointmentAfterUpdateResponse = new Appointment()
            {
                AppointmentId = 2,
                CustomerId = 2,
                AppointmentDateTimeStart = utcNow.AddHours(2),
                AppointmentDateTimeEnd = utcNow.AddHours(3),
                CreatedById = 2,
                LastUpdatedById = 2,
                CreatedDate = utcNow.AddHours(-1),
            ...
            appointmentRepository.Setup(method =>
                method.Get(2, default))
                    .ReturnsAsync(appointmentResultQueue.Dequeue);
            ...
            var expected = new AppointmentResponseType()
            {
                AppointmentId = 2,
                CustomerId = 2,
                AppointmentStartTime = utcNow.AddHours(2),
                AppointmentEndTime = utcNow.AddHours(3)
            };
        ...
        [Fact]
        public async Task UpdateAppointment_ShouldFail_CustomerDoesNotHaveAccess()
        {
            // arrange
            var appointmentRepository = new Mock<IAppointmentRepository>();
            var customerRepository = new Mock<ICustomerRepository>();
        ...
        [Fact]
        public async Task UpdateAppointment_ShouldFail_StartTimeEarlierThanNow()
        {
            // arrange
            var appointmentRepository = new Mock<IAppointmentRepository>();
            var customerRepository = new Mock<ICustomerRepository>();
        ...
        [Fact]
        public async Task UpdateAppointment_ShouldFail_AnotherAppointmentExistsInTheTimeSlot()
        {
            // arrange
            var appointmentRepository = new Mock<IAppointmentRepository>();
            var customerRepository = new Mock<ICustomerRepository>();
